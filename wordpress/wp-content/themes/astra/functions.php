...
error_log('Hi There');
        ...
        wp_remote_post('http://localhost/wordpress/submitpaper.php', [
            'body' => $fields
        ]);
...
$current_date = date('d-m-Y H:i:s'); // Changed date format to match MySQL datetime format
global $wpdb;

// Check if the current user matches the 'mid' and paper id matches
$user_mid = get_current_user_id(); // Assuming you are using WordPress and this fetches the current user's ID
$paper_id = $fields['paper_id']; // Assuming 'paper_id' is the identifier for the paper

// Updating the record
$output['success'] = $wpdb->update(
    'researchpaper',
    array(
        'date_of_last_update' => $current_date,
    ...
    array(
        ...
        'paper_id' => $paper_id // Condition to match the paper id
    )
);
